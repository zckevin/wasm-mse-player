...
async function run_player() {
  ...
  // mediaSource.duration = 63552 / 1000;
  // let sb = mediaSource.addSourceBuffer('video/mp4; codecs="avc1.640028, mp4a.40.2"');
  ...
  function appendBuffer(start, end) {
    ...
    console.log("sb.appendbuffer", tmp)
    sb.appendBuffer(tmp);
  }

	setInterval(() => {
	  if (fragments.length >= 4 && !sb.updating) {
	    if (fragments[0].atom_type === "ftyp" &&
...
/*
function run() {
  let worker = Comlink.wrap(new WasmWorker());
  ...
  worker.setFFmpegOnMessageCallback(Comlink.proxy(onFFmpegMsgCallback))
  worker.run(Comlink.proxy(onFragment))
}
*/
...
function run(videoAb) {
  ...
  let player = new WasmMsePlayer(videoAb.byteLength, readRequest, onFragment, onFFmpegMsgCallback);
  player.run();
}
