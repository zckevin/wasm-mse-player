...
import { OutputFileDevice, InputFileDevice } from "./emulated-device.js"
...
class WasmWorker {
  ...
  init(file_size, atomic_writer, onFragmentCallback, onFFmpegMsgCallback, sendReadRequest) {
  ...
  _ffmpeg_callback_delegate(utf8text) {
    assert(this.onFFmpegMsgCallback, "onFFmpegMsgCallback should not be undefined");
  ...
  runFFmpeg(Module) {
    ...
    try {
      // vi => void(int)
      var cb = Module.addFunction(this._ffmpeg_callback_delegate.bind(this), "vi");
